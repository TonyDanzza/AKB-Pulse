...
1. Скачай `AKB-Pulse-1.2.dmg` из раздела Releases на странице проекта.
2. Открой образ и перетащи `AKB.app` в папку «Программы».
3. При первом запуске открой приложение правой кнопкой → «Открыть» →
   «Открыть» ещё раз: оно подписано ad-hoc, без Developer ID, и обычный
