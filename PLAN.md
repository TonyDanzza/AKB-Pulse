...
- Текст: заголовок «iPhone почти разряжен», тело «iPhone TonyDanzza: 28%. Поставь на зарядку.» Звук по умолчанию. `interruptionLevel = .active` (уточнено в §26).
...
## 21. Фаза 4.4 — опрос каждые 5 с, пока телефон виден в сети (2026-09-09 18:35)

Лог `~/Library/Logs/AKB/akb.log` 18:27:58–18:28:23: телефон был виден через usbmuxd (ADD в 18:27:58), пользователь подключил зарядку ~18:28:10, события не было (телефон уже виден), таймер стоял на 300 с → зарядку увидели только при открытии popover (18:28:23, `refresh(rediscover:true)`). Иконка при этом обновляется корректно (§20 работает).

Правило: **если последний успешный опрос прошёл путём usbmuxd (телефон не спит), следующий опрос через 5 с.** Путём direct (телефон спит) — интервал из настроек; на питании — 5 с (как есть); неудача — 60 с (как есть).
- `BatteryStatus` (или отдельное поле результата провайдера) получает `source: .usbmuxd | .direct`; `nextPollDelay(interval:lastFailed:isOnPower:isAwake:)`: `lastFailed → min(60, interval)`; `isOnPower || isAwake → min(interval, 5)`; иначе `interval`. Тесты обновить (+2 кейса: awake → 5; awake и interval 30 → 5).
- `rescheduleIfNeeded()` уже есть — переключение вступает в силу сразу после опроса.
- Настройки: подпись пикера интервала переименовать в «Интервал опроса спящего iPhone» (en: "Poll interval while iPhone sleeps"), под ним `.caption .secondary`: «Пока iPhone не спит или на зарядке, опрос идёт каждые 5 с». Локализация en + ru.
- README: абзац «Расход батареи» дополнить одним предложением.
- Проверка: реальный режим, в логе при видимом телефоне интервалы ~5 с между строками `путём usbmuxd`; при `путём direct` — интервал из настроек. Коммит «Фаза 4.4: 5 с, пока iPhone не спит», DMG, заменить `/Applications/AKB.app` и запустить (как §18.3).

## 22. Правило: версия растёт с каждым дистрибутивом (2026-09-09)

Перед каждым `scripts/make-dmg.sh` / `package.sh` поднимать `MARKETING_VERSION` в `project.yml` (1.0 → 1.1 → 1.2 …; мелкая правка — следующий минор) и `CURRENT_PROJECT_VERSION` +1. Имя DMG — с версией. Бамп идёт в том же коммите, что и изменение. Текущее состояние: собранный `dist/AKB-1.0.dmg` содержит уже ~10 правок под номером 1.0 — при следующей сборке ставить **1.1** и дальше. Пока (по просьбе пользователя) ничего не собирать.

## 23. Фаза 4.5 — не терять адрес телефона (2026-09-09 18:45) → версия 1.1

Лог 18:37–18:42 (`~/Library/Logs/AKB/akb.log`): REMOVE → прямой путь не попал в окно за 42 с → «адрес забыт» → `arp -an` из приложения вернул пустоту («в arp нет записи для 34:10:be:d8:21:09») → адрес не найден, и так каждую минуту, кнопка «Обновить» упирается в то же. При этом Fable из терминала в тот же момент: `arp -an` содержит `192.168.1.11 at 34:10:be:d8:21:9`, `ping 192.168.1.11` отвечает (84 мс — телефон дремлет), порт 62078 открыт, `dns-sd -G v4 iPhone-Toni.local.` → 192.168.1.11 за 4 с. То есть телефон на месте, приложение потеряло его само.

### 23.1 Не забывать IP
- Кэш IP **не сбрасывать** после неудачного окна. Он инвалидируется только если: (а) usbmuxd/`addr` дал другой IP; (б) резолв по MAC или по имени дал другой IP; (в) IP не отвечал > 6 ч. Неудачное окно → просто следующая попытка по тому же IP через 60 с (плюс параллельно попытаться уточнить адрес, см. 23.2).
- В `RetryWindow` для «телефон спит» добавить дешёвую предпроверку: TCP-connect на порт 62078 с таймаутом 1 с каждые 2 с; полный `akb-direct battery` запускать только когда порт открылся. Так за окно 40 с делается ~20 проверок вместо 13 и меньше шансов пропустить волну.

### 23.2 Поиск адреса — три независимых пути, а не один arp
1. **mDNS по имени хоста.** Хранить `Prefs.lastKnownHostname[udid]`. Узнавать имя: при любом известном IP делать обратный резолв `getnameinfo` (mDNSResponder отвечает на `.local`) либо `dns-sd -q <reverse-ptr>`; результат `iPhone-Toni.local.` сохранить. Резолв вперёд — `getaddrinfo(hostname)` из Swift (без внешних процессов), таймаут 5 с; работает даже когда телефон дремлет (проверено).
2. **arp по MAC** — как сейчас, но: перед чтением таблицы послать `ping -c1 -W1000` на последний IP и broadcast‑`ping -c1 192.168.1.255` не нужен — достаточно пинга последнего IP; читать таблицу двумя способами: `/usr/sbin/arp -an` и, если вывод пуст, `sysctl` маршрутов (`CTL_NET, PF_ROUTE, 0, AF_INET, NET_RT_FLAGS, RTF_LLINFO`; если на macOS 26 отдаёт 0 байт — попробовать `NET_RT_DUMP` и фильтровать записи с `sockaddr_dl`). В лог писать **длину вывода arp и код** — сейчас непонятно, почему он пуст из приложения. Проверить гипотезу: запустить `arp -an` из установленного `/Applications/AKB.app` через `Process` в отладочной команде помощника или через `osascript`, сравнить с терминалом; если это разрешение «Локальная сеть» — при пустом arp писать в лог подсказку и показывать в popover строку «Разреши доступ к локальной сети в Системных настройках → Конфиденциальность» с кнопкой, открывающей `x-apple.systempreferences:com.apple.preference.security?Privacy_LocalNetwork`.
3. **usbmuxd `addr`** — как сейчас.
Порядок при потере: кэш → hostname → arp → addr. Любой успех обновляет кэш IP и hostname.

### 23.3 Проверка
- Воспроизвести сценарий: `defaults delete ru.tonydanzza.akb lastKnownIP` при спящем телефоне → приложение должно найти адрес через hostname или arp и снять заряд в течение 1–2 мин, лог показывает путь.
- Реальный режим 10 минут с лежащим телефоном без «адрес не найден» в логе.
- Тесты: hostname-резолвер с фейком, инвалидация кэша (не сбрасывается после неудачи; сбрасывается при другом IP), предпроверка порта в окне повторов.
- **Версия: `MARKETING_VERSION` 1.0 → 1.1, `CURRENT_PROJECT_VERSION` +1** (правило §22). Коммит «1.1: не терять адрес iPhone», `make-dmg.sh` → `dist/AKB-1.1.dmg`, заменить `/Applications/AKB.app`, запустить. В отчёте — версия из `Info.plist` установленного приложения.

## 24. Фаза 6 — здоровье батареи (2026-09-10) → версия 1.3

Заряд приложение показывает давно, а «сколько батарея вообще держит» — нет. Эти цифры iPhone отдаёт по той же доверенной паре, что и заряд: сервис `com.apple.mobile.diagnostics_relay` возвращает запись IORegistry `AppleSmartBattery`. Кабель не нужен, новых зависимостей у сборки тоже (`diagnostics_relay.h` лежит в той же libimobiledevice).

- Помощник `akb-direct` получает режим `health <ip|-> <udid>`: `-` идёт через usbmuxd (бодрствующий телефон), IP — прямым путём (спящий). Наружу печатается **только белый список** полей: `CycleCount`, `Voltage`, `InstantAmperage`, `Amperage`, `Temperature`, `TimeRemaining`, `IsCharging`, `ExternalConnected` и из `BatteryData` — `DesignCapacity`, `NominalChargeCapacity`, `FullChargeCapacity`. Серийный номер батареи, `IOReportLegend` и телеметрия не выводятся вовсе. Целые печатаются знаковыми: ток при разряде отрицательный.
- Что показываем: максимальную ёмкость (`NominalChargeCapacity / DesignCapacity`, округление — так же считает iOS), циклы, напряжение, ток и время чтения. Строка в окне строки меню «Здоровье 99 % ▏243 цикла» (правильные русские формы через plural-вариации) и раздел «Батарея» в настройках с кнопкой «Обновить».
- Раз в час, сразу после удачного чтения заряда; после неудачи следующая попытка не раньше чем через 10 минут. Неудача здоровья не трогает ни фазу, ни счётчик осечек, ни быстрый повтор — на экране заряда от неё не меняется ничего. Последние цифры хранятся в `UserDefaults` по UDID и показываются с датой: пока телефон спит, вчерашние 99 % честнее пустоты.
...
## 25. Фаза 7 — «отключи от зарядки» и выключатели уведомлений (2026-09-10) → версия 1.4

Две просьбы пользователя: сказать, когда iPhone на проводе дозарядился (до лимита зарядки iOS — у него 80 % — или до 100 %), и дать выключатели уведомлениям: общий и по одному на каждое.

- **Как телефон сообщает о лимите.** Проверено на живом iPhone 17 (iOS 26): у лимита `FullyCharged` в домене `com.apple.mobile.battery` остаётся `false`, и первые ~20 с после втыкания провода телефон тоже отвечает «питание есть, зарядки нет». Отличает лимит только `ChargerData.NotChargingReason` из записи IORegistry `AppleSmartBattery`: без провода и в момент втыкания — 128 (0x80), у лимита — с битом `0x01000000` (замеры 16777344 = 0x01000080 и 16777216 = 0x01000000). Помощник `akb-direct health` печатает это поле третьей группой белого списка.
- **Политика.** Новый чистый `Services/ChargeDonePolicy.swift` (как `AlertPolicy`, без ввода-вывода, целиком под тестами). Сеанс начинается с втыкания провода и кончается его отключением, уведомление за сеанс одно. «Провод есть, зарядки нет» после того, как зарядка шла, даёт `.needsConfirmation` — монитор спрашивает здоровье и подтверждает лимит по биту. Не подтвердилось или телефон не ответил — запасной путь: три подряд таких замера (15 с при опросе раз в 5 с) всё равно дают уведомление. 100 % (`FullyCharged` или заряд ≥ 100) шлётся сразу, даже если телефон воткнули уже полным. Телефон, воткнутый выше лимита, молчит: зарядка при нём не шла.
- **Уведомление.** `NotificationService.postChargeDone`: «iPhone заряжен до 80 %» / «iPhone заряжен полностью», `interruptionLevel = .active` — это не тревога.
- **Настройки.** `notificationsEnabled` (общий) и `notifyChargeDone`, оба по умолчанию включены. Раздел «Уведомления»: общий переключатель, «Низкий заряд» с порогом и повтором, «Отключить от зарядки» с подписью. Дочерние контролы гаснут вместе с общим. Общий выключатель — второй рубеж и в самом `NotificationService`. Высота окна пересчитана замером: 916 → 1031 pt.
- **Порог запасного пути.** Лимит зарядки в iOS ставится только в диапазоне 80…100 %, поэтому запасной путь (`ChargeDonePolicy.minimumLimitPercent = 80`) молчит ниже 80 %: остановка зарядки на 50 % — это пауза по нагреву, а не дозарядка. Подтверждённый по биту лимит и 100 % шлются на любом проценте.
- **Тесты:** 249 → 276. Живая проверка: помощник на телефоне у лимита отдал `NotChargingReason: 16777216`; Debug-сборка на настоящем телефоне (80 %, на проводе, зарядка не шла) за три минуты не сказала ничего — как и задумано; фейковый режим `AKB_FAKE_CHARGE_DONE` показал оба уведомления по одному разу.
- Версия по правилу §22: `MARKETING_VERSION` 1.4, `CURRENT_PROJECT_VERSION` 9, `make-dmg.sh` → `dist/AKB-Pulse-1.4.dmg`.

## 26. 1.4.1 — уведомления уважают «Не беспокоить» (2026-09-10)

- `NotificationService.postLowBattery`: уровень прерывания понижен до `.active`, как у «отключи от зарядки». Прежний уровень пробивал «Не беспокоить» и фокусы macOS, а разряженный ночью iPhone подождёт до утра.
- Своих «тихих часов» в приложении нет и не будет: тишиной управляет система, как у остальных приложений. Версия по правилу §22: `MARKETING_VERSION` 1.4.1, `CURRENT_PROJECT_VERSION` 10.
